#!/bin/bash

# filepath: /home/meciek/Desktop/gs_web_app/setup_and_run.sh

function build_messages() {
    echo "Building ROS 2 messages..."
    cd gs_interfaces || exit
    python generate_ros2_messages.py
    if [ $? -ne 0 ]; then
        echo "Failed to generate ROS 2 messages."
        exit 1
    fi

    cd ..

    colcon build --packages-select gs_interfaces
    if [ $? -ne 0 ]; then
        echo "Failed to build gs_interfaces package."
        exit 1
    fi
    echo "ROS 2 messages built successfully."
}

function run_app() {
    echo "Starting the app (npm run dev)..."
    cd app || exit
    npm run dev
    cd ..
}

function run_server() {
    echo "Starting the server (python main.py)..."
    cd server || exit
    python main.py
    cd ..
}

function run_custom_messages() {
    echo "Running custom messages (python sim_nodes/run.py)..."
    python sim_nodes/run.py
}

function generate_mavlink_definitions() {
    echo "Generating MAVLink definitions using setup.sh..."
    cd mavlink|| exit
    ./setup.sh simba message_definitions/v1.0/simba.xml
    if [ $? -ne 0 ]; then
        echo "Failed to generate MAVLink definitions."
        exit 1
    fi
    cd ..
    echo "MAVLink definitions generated successfully."
}

function build_msgs() {
    generate_mavlink_definitions
    build_messages
    wait
}

function run_and_build_all() {
    generate_mavlink_definitions
    build_messages
    run_app &
    run_server &
    run_custom_messages &
    wait
}

function run() {
    run_server &
    run_app &
    wait
}

function run_with_test_msgs() {
    run_app &
    run_server &
    run_custom_messages &
    wait
}

function show_help() {
    echo "Usage: $0 [option]"
    echo "Options:"
    echo "  build_messages           Build ROS 2 messages and gs_interfaces package"
    echo "  run_app                  Start the app (npm run dev)"
    echo "  run_server               Start the server (python main.py)"
    echo "  run_custom_messages      Run custom messages (python sim_nodes/run.py)"
    echo "  generate_mavlink         Generate MAVLink definitions using setup.sh"
    echo "  help                     Show this help message"
}

# Main script logic
if [ "$#" -lt 1 ]; then
    show_help
    exit 1
fi

case $1 in
    build_messages)
        build_messages
        ;;
    run_app)
        run_app
        ;;
    run_server)
        run_server
        ;;
    run_custom_messages)
        run_custom_messages
        ;;
    generate_mavlink)
        generate_mavlink_definitions
        ;;
    
    run_and_build_all)
        run_and_build_all
        ;;
    run_all)
        run_all
        ;;
    run_with_test_msgs)
        run_with_test_msgs
        ;;
    run)
        run
        ;;
    help)
        show_help
        ;;
    build_msgs)
        build_msgs
        ;;
    *)
        echo "Invalid option: $1"
        show_help
        exit 1
        ;;
esac